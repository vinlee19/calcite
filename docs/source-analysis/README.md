...
> 基线 commit：`111030383` · 共 20 篇 + 第 21 篇综合篇 + 67 张手写 SVG（架构图 / 类图 / 流程图 / 时序图 / 对比矩阵）。
...
> 例外说明：第 21 篇是一篇「**综合篇**」。它按"特性"纵切，把子查询 / CTE / 开窗函数各自走一遍五阶段全链路——这是唯一带"流程叙事"色彩的一篇。但它与 [`docs/calcite-guide/`](../calcite-guide/README.md) 入门流程的差异是**源码锚点深度**（具体类/方法/关键一步），与 01–20 的差异是**横切综合**而非单点机制；每个阶段的通用机制仍只一句话 + 链接到主讲篇。
...
| 21 | [三道硬菜的全链路：子查询 / CTE / 开窗函数](articles/21-feature-pipeline.md) ☆ | 综合篇：三个特性各走一遍五阶段，列出关键一步 |

★ = IR 重心篇；☆ = 综合篇（按特性纵切，串联前 20 篇）。
...
- **特性贯穿路线（收官）**：02 → 08 → 12 → 16 → 21 — 先建立 IR/sql2rel/规则/codegen 基础，再用第 21 篇把子查询、CTE、开窗三个难特性的全链路串起来。

建议先读 01、02 建立全局观，再按兴趣选一条主线深入；最后用第 21 篇做一次"特性级"的综合串联。
